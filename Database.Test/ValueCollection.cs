...
namespace Database.Test
{
    ...
    public class ValueCollection<T> : Collection<T>, IEquatable<ValueCollection<T>>, IFormattable
    {
        ...
        public ValueCollection (IEqualityComparer<T>? equalityComparer = null)
            : this(new List<T>(), equalityComparer) { }

        public ValueCollection (IList<T> list, IEqualityComparer<T>? equalityComparer = null)
            : base(list) => this.equalityComparer = equalityComparer ?? EqualityComparer<T>.Default;
        ...
        {
            return obj is { } && (ReferenceEquals(this, obj) || obj is ValueCollection<T> coll && Equals(coll));
        }

        public override int GetHashCode () =>
            unchecked(Items.Aggregate(0, (current, element) =>
                (current * 397) ^ (element is null ? 0 : equalityComparer.GetHashCode(element))));
